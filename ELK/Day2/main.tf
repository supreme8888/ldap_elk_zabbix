...
resource "google_compute_address" "int_elk_ip" {
  name         = "internal-elk-ip"
...
resource "google_compute_instance" "elk" {
  ...
  network_interface {
    network    = "elk-vpc"
    subnetwork = google_compute_subnetwork.public.id
    network_ip = google_compute_address.int_elk_ip.address
  ...
  metadata_startup_script = templatefile("script.sh", {
    elk_ip  = google_compute_address.int_elk_ip.address
    surname = "vvv"
  })
}
...
resource "google_compute_instance" "elk_client" {
  ...
  metadata_startup_script = templatefile("script1.sh", {
    elk_ip  = google_compute_instance.elk.network_interface.0.network_ip
    surname = "vvv"
  })
